...
def localizar(df,logradouro,num,area=None,modo='nome'):
    ...
    def interpolar(num,pontos):
        ...
        delta_x = float(p_max.iloc[0]['LESTE'].replace(',','.')) - float(p_min.iloc[0]['LESTE'].replace(',','.'))
        delta_y = float(p_max.iloc[0]['NORTE'].replace(',','.')) - float(p_min.iloc[0]['NORTE'].replace(',','.'))
    ...
    if modo == 'nome':
        logradouro = processar_logradouro.limpar(logradouro)
        logradouro_selecionado = area_selecionada[(area_selecionada['NOME_LOGRA']==logradouro)]
    else:
        try:
    ...
    if modo == 'codigo':
        logradouro_selecionado = area_selecionada[(area_selecionada['ID_LOGRADO']==logradouro)]
    if modo == 'cep':
        logradouro_selecionado = area_selecionada[(area_selecionada['CEP']==logradouro)]
    ...
    if len(logradouro_selecionado) == 0:
        ...
        logradouro_aproximado = process.extractOne(logradouro, choices=lista_logradouros, score_cutoff=LIMITE_FUZZY)
        if logradouro_aproximado == None:
            return {'X':'','Y':'','GEOLOG':'LOGRADOURO NAO ENCONTRADO'}
        else:
            logradouro_selecionado = area_selecionada[(area_selecionada['NOME_LOGRA']==logradouro_aproximado[0])]
            texto_log = 'LOGRADOURO APROXIMADO / '

    # Busca o número do imóvel
    selecao_imoveis = logradouro_selecionado[logradouro_selecionado['NUMERO_IMO']==num]
    if len(selecao_imoveis) > 0: # Correspondência exata encontrada para o número do imóvel
        return {'X':selecao_imoveis.iloc[0]['LESTE'],'Y':selecao_imoveis.iloc[0]['NORTE'],'GEOLOG':texto_log+'NUMERO EXATO'}
    else:
        ...
        selecao_imoveis = logradouro_selecionado[logradouro_selecionado['PAR']==num_par]
        proximos = selecao_imoveis.iloc[(selecao_imoveis['NUMERO_IMO']-num).abs().argsort()[:2]]
        if len(proximos) < 2:
            # Retorna coordenadas em branco se não houver ao menos 2 imóveis para interpolar
            return {'X':'','Y':'','GEOLOG':texto_log+'POUCOS PARAMETROS PARA INTERPOLACAO'}
        ...
        (abs(proximos.iloc[1]['NUMERO_IMO']-num)>LIMITE_NUMERO)):
            ...
            if (val['X']=='') or (val['Y']==''):
                ...
                return {'X':val['X'],'Y':val['Y'],'GEOLOG':texto_log+'NUMERO INTERPOLADO'}
